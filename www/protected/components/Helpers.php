...
class Helpers
{
     ...
     * @param string $method round up or down. Default is 'up'
    ...
    public static function roundTime($time, $module, $itemFormat, $method = 'up')
    {
        ...
        $rounded = floor($itemValue / $module) + (($method == 'up') ? 1 : 0);
        switch ($itemFormat) {
            case 's':
    ...
    {
        return self::roundTime($time, $module, $itemFormat, 'down');
    }

    public static function roundTimeUp($time, $module, $itemFormat)
    {
        return self::roundTime($time, $module, $itemFormat, 'up');
    }

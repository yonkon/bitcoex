...
ORDER BY date ASC");
    ...
    foreach ($currentOrders as $ord) {
      ...
      if (!($this->rest > 0)) {
        throw new Exception("Assertion failed: Order ID#{$this->id} has a negative rest value");
      }
      if ($ord->rest == 0) {
        continue;
      }
...
//        assert($reservedSuccess);
  ...
  public function cancel()
  {
    ...
    if (!$this->srcWallet->save()) {
      $this->addError('src_wallet',
        Yii::t('error', 'Невозможно изменить данные по кошельку') .
        '.\n' .
        Yii::t('error', 'Ответ сервера:') .
        join('\n', $this->srcWallet->getErrors())
      );
    } else {
      $this->save();
    }
